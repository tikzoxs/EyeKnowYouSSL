...
def main(unused_argv):
	...
	base_model = keras.applications.MobileNetV2(include_top=False, weights=None, input_tensor=None, input_shape=(144,256,3), pooling='avg')
	x = base_model.output
	...
	eyeknowyou_supervised_model.load_weights(ckpt_supervised)

	generator = iris_data_loader.eyeknowyouTestGenerator()

	for x in range(1,20):
		(image, points, img) = next(generator)
		point = eyeknowyou_supervised_model.predict(image)
		x = int(point[0][1]*256)
		y = int(point[0][0]*144)
		img = np.asarray(img)
